...
class TestSyncService:
    ...
    def test_sync_runner_found_in_github(self, test_db: Session, mock_settings):
        ...
        runner = Runner(
            ...
            runner_group_id=1,
            labels=json.dumps(["self-hosted", "linux", "test"]),
    ...
    def test_sync_pending_runner_with_expired_token(
        ...
        # Simulate runner created 2 hours ago (JIT config expires after 1 hour)
        runner.created_at = datetime.now(timezone.utc) - timedelta(hours=2)
        test_db.add(runner)
        test_db.commit()
        test_db.refresh(runner)
    ...
    def test_sync_pending_runner_with_valid_token_unchanged(
        ...
        # Runner created just now - JIT config still valid (< 1 hour old)
        test_db.add(runner)
        test_db.commit()
        test_db.refresh(runner)
